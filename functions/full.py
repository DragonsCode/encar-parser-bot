...
async def fetch_car_full_info(car, exchange_rate, db: DBApi, sem: asyncio.Semaphore):
    """Обрабатывает полную информацию об автомобиле и записывает в БД, используя переданную сессию."""
    async with sem:
        try:
...
async def parse_full_car_info(max_pages: int = None):
    ...
    # Создаём одну сессию для всей операции
    async with DBApi() as db:
        for car_type in ['kor', 'ev']:
            ...
            async for page_cars in parse_cars(car_type, max_pages):
                ...
                tasks = [fetch_car_full_info(car, exchange_rate, db, sem) for car in new_cars]
